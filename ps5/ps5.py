...
class PhraseTrigger(Trigger):
    """ Abstract class for phrase trigger that takes in a phrase as a constructer input"""

    def __init__(self, phrase):

        self.phrase = phrase.lower().spilt()
    ...
    def is_phrase_in(self, story):
        ...
        #check if the words in the phrase are present in the text
        for word in self.phrase:
            if word not in story:
                return  False

        story = "".join(story)
        if "".join(self.phrase) in story:
            return  True
        else:
            return False
...
class TimeTrigger(Trigger):
    ...
    def __init__(self,datetime_str):
        self.time = datetime.strptime(datetime_str,"%d %b %Y %H:%M:%S")
...
class AndTrigger(Trigger):
    ...
    def __init__(self,trigger1, trigger2):
        self.trigger_1 = trigger1
        self.trigger_2 = trigger2
...
def read_trigger_config(filename):
    ...
    for element in lines:
        ...
        if contents[0] == "ADD":
            trigger_list.extend([triggers[trigger] for trigger in contents[1:]])
        else:
            if contents[1] == "AND" or contents[1] == 'OR':
                triggers[contents[0]] = trigger_dict[contents[1]](triggers[contents[2]],triggers[contents[3]])
            else:
                triggers[contents[0]] = trigger_dict[contents[1]](contents[2])
